...
static int on_sign_show(void *session, DynamicViewCtx *view) {
    ...
    if (msg->coin.type == COIN_TYPE_BITCOIN) {
	...
	if (msg->brc20_type == BRC20_NFT) {
		...
		view->coin_type = COIN_TYPE_BRC20;
		...
		if (msg->brc20_type == BRC20_INSCRIBE || msg->brc20_type == BRC20_SEND) {
		...
		if (msg->brc20_type == BRC20_SEND || msg->brc20_type == BRC20_INSCRIBE) {
