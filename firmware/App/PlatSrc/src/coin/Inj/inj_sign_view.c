...
static int on_sign_show(void *session, DynamicViewCtx *view) {
...
#if 1
    db_msg("-------------------------------");
    db_msg("msg->operation_type:%x", (char) msg->operation_type);
    ...
    if ((char) msg->operation_type == OP_TYPE_TRANSFER) {
        ...
        db_msg("msg->action.sendCoins.account_number:%s", msg->action.sendCoins.account_number);
        db_msg("msg->action.sendCoins.amount:%s", msg->action.sendCoins.amount);
        db_msg("msg->action.sendCoins.fee:%s", msg->action.sendCoins.fee);
...
//        db_msg("bignum2double ret:%d,send_value:%f,tmpbuf:%s", ret, send_value, tmpbuf);
        ...
        view_add_txt(TXS_LABEL_MAXID, "Injective");
        ...
        view_add_txt(TXS_LABEL_TOTAL_VALUE, "INJ");
    } else if ((char) msg->operation_type == OP_TYPE_DAPP) {
