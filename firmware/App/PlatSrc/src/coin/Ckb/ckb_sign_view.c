...
static int on_sign_show(void *session, DynamicViewCtx *view) {
	...
	if(msg->operation_type == OP_TYPE_TRANSFER){
		...
		int first_output_offset = 64;//res_getInt(MK_txs_sign, "first_output_offset", 0);
		int output_item_height = 64;//res_getInt(MK_txs_sign, "output_item_height", 0);
	    ...
	    view_add_txt(TXS_LABEL_TOTAL_VALUE, coinConfig->symbol);
		...
		if (!(msg->outputs[i].is_change_address & CKB_OUTPUT_FLAG_CHANGE)) {
			...
			if (strlen(msg->outputs[i].address) > CKB_SHORT_ADDR_LEN) {
				omit_string(tmpbuf, msg->outputs[i].address, 11, 26);
				view_add_txt(TXS_LABEL_TOTAL_VALUE, tmpbuf);
			}else{
				view_add_txt(TXS_LABEL_TOTAL_VALUE, msg->outputs[i].address);
			}
		...
		if (input_n) {
			for (i = 0; i < msg->input_n; i++) {
				...
				if (strlen(msg->inputs[i].address) > CKB_SHORT_ADDR_LEN) {
					omit_string(tmpbuf, msg->inputs[i].address, 11, 26);
					view_add_txt(TXS_LABEL_TOTAL_VALUE, tmpbuf);
				}else{
					view_add_txt(TXS_LABEL_TOTAL_VALUE, msg->inputs[i].address);
				}
		...
		if (change_count > 0) {
			for (i = 0; i < msg->output_n; i++) {
			    ...
			    view_add_txt(TXS_LABEL_TOTAL_VALUE, res_getLabel(LANG_LABEL_TXS_CHANGE_TITLE));
		...
		if (strlen(msg->outputs[0].address) > CKB_SHORT_ADDR_LEN) {
			omit_string(tmpbuf, msg->outputs[0].address, 11, 26);
