...
int update_menu(MENU_SET_CFG menu[4], type_uname param_list[4], int index, char pMenuText[4][20], type_uname *param, const char *uname){
    if (index >= 4) {
        db_error("index err");
        return -10;
    }
    ...
    param_list[index].type = param->type;
...
int AddressTypeWin(type_uname *param) {
    ...
    if (IS_BTC_COIN_TYPE(param->type)) {
        ...
        if (!strcmp(addr_uname, "BTC")) {
            ...
            ret = update_menu(menu, param_list, 0, pMenuText, param, "BTC");
            if (ret != 0) {return ret;}

            ret = update_menu(menu, param_list, 1, pMenuText, param, COIN_UNAME_BTC2);
            if (ret != 0) {return ret;}

            ret = update_menu(menu, param_list, 2, pMenuText, param, COIN_UNAME_BTC3);
            if (ret != 0) {return ret;}

            ret = update_menu(menu, param_list, 3, pMenuText, param, COIN_UNAME_BTC4);
            if (ret != 0) {return ret;}

            index = 4;
        } else if (!strcmp(addr_uname, "LTC")) {
            const char pMenuText[2][20] = {"Native SegWit", "Legacy"};
            ret = update_menu(menu, param_list, 0, pMenuText, param, "LTC");
            if (ret != 0) {return ret;}

            ret = update_menu(menu, param_list, 1, pMenuText, param, COIN_UNAME_LTC2);
            if (ret != 0) {return ret;}

            index = 2;
        } else if (!strcmp(addr_uname, "BCH")) {
            const char pMenuText[2][20] = {"CashAddr", "Legacy"};
            ret = update_menu(menu, param_list, 0, pMenuText, param, "BCH");
            if (ret != 0) {return ret;}

            ret = update_menu(menu, param_list, 1, pMenuText, param, COIN_UNAME_BCH2);
            if (ret != 0) {return ret;}
        ...
        ret = update_menu(menu, param_list, 0, pMenuText, param, COIN_UNAME_SOL2);
        if (ret != 0) {return ret;}

        ret = update_menu(menu, param_list, 1, pMenuText, param, "SOL");
        if (ret != 0) {return ret;}
